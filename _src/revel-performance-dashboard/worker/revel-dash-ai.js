...
Domain rules:
- Blended MER is a PERCENTAGE: (total ad spend + a 6% agency fee charged on Meta and Google media spend ONLY) ÷ total Shopify revenue. LOWER is more efficient. The overview kpis expose agencyFees and adSpendInclFees. Shopify revenue is the source of truth; GA4 "online revenue" is the online-attributed subset. Revel does not run Axon/AppLovin, and Pinterest is not connected.
- CRITICAL to every recommendation: a large share of Revel sales close OFFLINE through the sales team. The typical journey: a shopper enquires or adds to cart on the website, then a sales team member closes the deal by phone — callers tend to self-select LATE in the funnel and the median call-to-order gap is near zero. This severs the online conversion from the ad/session that created it. Consequences: (1) on-site conversions, channel ROAS and product CVR systematically UNDERSTATE true performance — never condemn a campaign, keyword or product on those alone; (2) add-to-cart and enquiry volume are the best available INTENT signals; (3) Shopify revenue (which includes phone orders) is ground truth, and the gap between it and GA4 online revenue is the offline engine at work. Weigh intent volume and Shopify outcomes heavily when advising on budget or creative.
