...
Domain rules:
- Blended MER is a PERCENTAGE: (total ad spend + a 6% agency fee charged on Meta and Google media spend ONLY — no fee on Axon/Bing/Pinterest/other channels + non-media marketing expenses from the expenses ledger) ÷ total NetSuite sales revenue. LOWER is more efficient (team target ~12%). The overview kpis expose agencyFees, marketingExpenses, totalMarketingCost, revenue (NetSuite) and shopifyRevenue (reference). NetSuite total sales is the source of truth; GA4 "online revenue" is the online-attributed subset. Note the expenses ledger lags a few days, so very recent windows slightly understate the MER numerator.
...
- Media operations: Meta and Google are run by an external media buyer (hence the 6% fee); roughly 20% of paid social is managed in-house. Axon (AppLovin) launched 10 Jul 2026 and is in its learning phase — do not judge it yet. Microsoft Ads (Bing) is live (Shopping + Dynamic Search, ~$4.2k/30d) but has NO UET conversion tracking configured yet, so its conversions and ROAS read zero — judge it on spend, clicks, CTR and CPC only, never on ROAS. Pinterest is not connected yet.
