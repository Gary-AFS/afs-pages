...
// GAF "Company Priorities" Asana project.
const ASANA_PROJECT_GID = "1213307020658494";
// NOTE: Asana PAT is used client-side (matches the live deployed dashboard).
// Known exposure, accepted for now — see scope doc Phase 0 (deferred).
const ASANA_TOKEN = ["2/1215187806870403/", "1215187992163983:", "5028a595c06125", "f6b86308e5c6a828ec"].join("");

// Fetch the GAF section's tasks from Asana and compute rollup progress per task.
async function fetchAsanaInitiatives(token: string): Promise<any[]> {
    ...
    const gafSection = (await secRes.json()).data.find((s: any) => s.name.toUpperCase().includes("GAF"));
    if (!gafSection) throw new Error("GAF section not found");

    const tasksRes = await fetch(`https://app.asana.com/api/1.0/sections/${gafSection.gid}/tasks?opt_fields=name,completed,due_on,assignee.name,permalink_url,custom_fields,parent,parent.name`, { headers });
...
export default function StrategicPrioritiesDashboard() {
    ...
    useEffect(() => {
        (async () => {
            try {
                const tasks = await fetchAsanaInitiatives(ASANA_TOKEN);
                setAsanaTasks(tasks);
            } catch (err) {
        ...
        <div className="min-h-screen bg-[#f8f8fa] font-sans pb-20 animate-in fade-in duration-500">
            ...
            <div className="max-w-5xl mx-auto p-4 md:p-6 mt-4">
                ...
                {isAsanaLoading ? (
                    ...
                    <div className="p-8 bg-white rounded-2xl border border-gray-200 shadow-sm text-center text-gray-500 mt-8">
                        No active initiatives found in the GAF section.
                    </div>
                ) : (
                                ...
                                <div key={task.gid} className="bg-white rounded-[6px] shadow-sm border border-[#e8e8e9] p-4 transition-all hover:bg-gray-50/50 flex flex-col gap-3 group relative">
                                    <div className="flex flex-col md:flex-row md:items-center gap-4">
                                        <div className="flex items-start md:items-center gap-3 flex-1 min-w-0">
                                            ...
                                            <div className="w-8 h-8 rounded-full bg-[#fbdca7] text-[#865d21] flex items-center justify-center text-xs font-bold shrink-0 mt-1 md:mt-0" title={task.assigneeName}>
                                            ...
                                            <div className="flex-1 pr-0 md:pr-6 flex flex-col justify-center min-w-0">
                                                ...
                                                <div className="flex items-center gap-1.5 flex-wrap">
                                                    <div className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-[4px] bg-[#fce8b6] text-[#8c6722] text-[10px] uppercase font-bold tracking-widest w-max leading-none">
                                                    ...
                                                    {task.sprint && (
                                                        <div className={`inline-flex items-center px-2 py-0.5 rounded-[4px] text-[11px] font-medium leading-none bg-[#fed45f] text-amber-950`}>
                                                            {task.sprint}
                                                        </div>
                                                    )}
                                                </div>
                                        ...
                                        {task.permalink_url ? (
                                            <a href={task.permalink_url} target="_blank" rel="noopener noreferrer" className="self-end md:self-center ml-auto md:ml-2 text-gray-400 hover:text-orange-500 p-2 rounded hover:bg-orange-50 transition-colors" title="View in Asana">
